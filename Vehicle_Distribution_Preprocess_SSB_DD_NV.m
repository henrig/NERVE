...
% figure(1);clf
% subplot(2,1,1)
% bar(yrs,[squeeze(nansum(nansum(nansum(YTD(:,:,:,7),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,6),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,5),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,4),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,3),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,2),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,1),2),3),4))],'stacked')
% legend('pre-Euro','Euro 1','Euro 2','Euro 3','Euro 4','Euro 5','Euro 6')
% title('Total nasjonal Kjørelengde per Eurokategori ')
% ylabel('Km')
% 
% subplot(2,1,2)
% bar(yrs,[squeeze(nansum(nansum(nansum(YTD(:,:,:,7),2),3),4)),...
...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,3),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,2),2),3),4)),...
% squeeze(nansum(nansum(nansum(YTD(:,:,:,1),2),3),4))])
% legend('pre-Euro','Euro 1','Euro 2','Euro 3','Euro 4','Euro 5','Euro 6')
% title('Total nasjonal Kjørelengde per Eurokategori ')
% ylabel('Km')
% 
...
% 
% % % -------------------------------------------------------------------------
% % % Rearrange age, nybiltype TO age*nybiltype
% % % -------------------------------------------------------------------------
% % % Age is "backwards" in the SSB data i.e. Euro 6 == 1
% % [Nkomm, NyType,Age] = size(NV);
% % NVtemp              = zeros(Nkomm,NyType*Age);
% % TDtemp              = zeros(size(NVtemp));
% % 
% % NVtemp = reshape(NV,size(NV,1),size(NV,2)*size(NV,3));
% % TDtemp = reshape(TL,size(TL,1),size(TL,2)*size(TL,3));
% % 
% % %--------------------------------------------------------------------------
% % % Join into model classes.
% % %--------------------------------------------------------------------------
% % sheet        = 'SSB_IDtags';
% % TS           = readtable(exchfile,'Sheet',sheet);
% % TFmatix      = unique(TS.ModelID(:));
% % if debug_mode; fprintf('using: %s\n Sheet: %s\n For reorgaizing SSB NV and TD\n',exchfile,sheet); end
% % sheet        = 'Model_IDtags';
% % TM           = readtable(exchfile,'Sheet',sheet);
% % if debug_mode; fprintf('using: %s\n Sheet: %s\n For reorgaizing SSB NV and TD\n',exchfile,sheet); end
% % 
% % 
% % for i=1:size(TFmatix,1)
% %     I   =  TS.ModelID == TFmatix(i);
% %     ie  =  TS.SSB_Pos(I);
% %     if sum(I)==1
% %         if debug_mode; fprintf(' %04i_%s <--- %s :: %i\n',TFmatix(i),char(TM.ModelName(TFmatix(i))),char(TS.SSB_NAME(I)),nansum(NVtemp(:,I))); end
% %         sNV(:,TFmatix(i))   = NVtemp(:,I);
% %         sTD(:,TFmatix(i))   = TDtemp(:,I);
% %     elseif sum(I)>1
% %         sNV(:,TFmatix(i))   = nansum(NVtemp(:,I),2);
% %         sTD(:,TFmatix(i))   = nansum(TDtemp(:,I),2);
% %         for n=1:length(ie)
% %             if debug_mode; fprintf(' %04i_%s <--- %s :: %i\n',TFmatix(i),char(TM.ModelName(TFmatix(i))),char(TS.SSB_NAME(ie(n))),nansum(nansum(NVtemp(:,ie(n))))); end
% %         end
% %     end
% % end
% % 
% % TD=sTD;
% % NV=sNV;
% % fprintf('sNV  With Total Vehicles %i\n',sum(sum(NV))) 
% % fname         = sprintf('%s%s',CPF,'v3.mat');
% % save(fname,'TD','NV')
% % fprintf('Wrote file: \n %s\n', fname)
% 
% 
% % 
% % 
% % for komm=1:length(kmnes2)
% %     I1=Data(:,1)==kmnes2(komm);
% %     % extract the municipality
% %     BP=Data(I1,:);
% %     ST.ID(komm) = BP(1,1);
% %     ST.FY(komm) = BP(1,2);
% %     ST.AR(komm) = BP(1,11);
% %     ST.N (komm) = nansum(BP(:,6));
% %     
% %     remo = zeros(length(BP),1);
% %     for bp=1:52
% %         Ibt=BP(:,3)==bp;
% %         % loop "produksjonsår"
% %         for k=1:7
% %             if komm==1; ST.VehType(t)=bp; ST.VehRegYrID(t)=k; ST.EU(t)=7-k; t=t+1; end
% %             Ieu=BP(:,4)==k;
% %             I=Ibt&Ieu;
% %             if sum(I)>1
% %                 idx = find(I);
% %                 % kmnes(komm)
% %                 % kommkode;fylke;nybiltype;regaar;snittkomm;antallkomm;snittfylke;antallfylke;snittland;antalland;stataar
% %                 %    5054    17      51      3         NaN       1       36191       17         36297      447     2016
% %                 BP(idx(1),6) = sum(BP(idx,6));
% %                 for am = 2:length(idx)
% %                     remo(idx(am)) = 1;
% %                 end
% %             end
...
% %                 % 4) final try give it an average of cars in kommune
% %                 if ~isnan(BP(I,5))      %1)
% %                     TL(komm,bp,k) = BP(I,6).*BP(I,5);
% %                     VL(komm,bp,k) = BP(I,5);
% %                     c(1)          = c(1)+BP(I,6);
% %                 elseif ~isnan(BP(I,7))  %2)
% %                     TL(komm,bp,k) = BP(I,6).*BP(I,7);
% %                     VL(komm,bp,k) = BP(I,7);
% %                     c(2)          = c(2)+BP(I,6);
% %                 elseif ~isnan(BP(I,9))  %3)
% %                     TL(komm,bp,k) = BP(I,6).*BP(I,9);
% %                     VL(komm,bp,k) = BP(I,9);
% %                     c(3)          = c(3)+BP(I,6);
% %                 else                    %4)
% %                     %warning(sprintf('Could not match Vehicle: %i %s',bp,char(regaar(k))))
% %                     c(4)=c(4)+BP(I,6);
% %                     TL(komm,bp,k)=BP(I,6).*nanmedian(BP(:,9));
% %                     VL(komm,bp,k)=nanmedian(BP(:,9));
% %                 end
% %             elseif sum(I)>1
% %                 NV(komm,bp,k)=sum(BP(I,6),1);
% %                 error(sprintf('found %i vehicles of %i,%i in municipality %04i \n',length(find(I)),bp,k,BP(1,1)))
% %                 % error(sprintf('TOO MANY VEHICLE CLASSES IDENTIFIED AS ONE: %i %s\n',bp,char(regaar(k))))
% %             else
% %                 NV(komm,bp,k)=0;
% %             end
% %         end
...
% %     Distkomm(komm) = squeeze(nansum(nansum(TL(komm,:,:),2),3));
% %     if debug_mode
% %         fprintf(rfid,'%04i;%i;%i',kmnes(komm),Numkomm(komm),round(Distkomm(komm)));
% %         fprintf('Kommune: %04i has %i cars TotDist %i \n',kmnes(komm),Numkomm(komm),round(Distkomm(komm)))
% %     end
% %     
% % end
% % 
...
% %     idx3 = kommuner2019==komm(i);
% %     idx4 = kommuner==komm(i);
% %
% %
% %     if sum(idx2)==1
% %         fprintf('Kommune: %04i still exists\n',komm(i))
% %     else
% %         % kommune does not exist
% %         % check if it existed in 2019
% %         if sum(idx3)==1
% %             fprintf('Kommune: %04i changed to %04i in 2020\n',komm(i),kommuner(idx3))
% %             %T.kommkode(idx) = kommuner(idx3);
% %         else
% %             fprintf('\t Kommune: %04i did not exist in 2019\n',komm(i))
% %
% %
% %         end
% %
